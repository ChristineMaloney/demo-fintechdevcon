...
//
// The mock environment is the exception: it is unauthenticated, so against
// it `getAccessToken()` returns null and no credentials are read at all.
// See `requiresAuth()`.
//
...
// The mock environment accepts requests with no Bearer token, so nothing
// needs to be signed or exchanged when a call targets it.
const MOCK_HOSTNAME = 'api-mock.payments.jpmorgan.com';

/**
 * Whether calls to `url` need an access token. False only for the mock
 * environment — CAT and PROD always require OAuth.
 *
 * @param {string} url Any URL on the target API (base URL or full path).
 * @returns {boolean}
 */
export function requiresAuth(url = API_BASE_URL) {
  try {
    return new URL(url).hostname !== MOCK_HOSTNAME;
  } catch {
    // An unparseable override is not something to silently treat as mock.
    return true;
  }
}
 ...
 *
 * Returns null when API_BASE_URL points at the unauthenticated mock
 * environment — callers should omit the Authorization header in that case.
 *
...
// import { request } from 'undici';
//
// const token = await getAccessToken();  // null against the mock env
...
//
// Pointing at a different host than API_BASE_URL? Gate the token fetch on
// requiresAuth(thatUrl) so the mock env stays credential-free.
