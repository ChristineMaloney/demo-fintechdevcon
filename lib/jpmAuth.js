...
// Auth and the API surface are configured independently, so it is easy to
// point the API at a mock while every token exchange still goes to
// production IDAnywhere — in which case the mock is never reached at all,
// because auth runs first and fails first.
export function describeTargets() {
  ...
  return {
    apiHost,
    tokenHost,
    split: tokenHost === new URL(DEFAULT_TOKEN_URL).host && apiHost.includes('mock'),
  };
}
