...
// already caches one across calls, which is the whole point of it. Against
// the mock environment no token is fetched or sent at all — see REQUIRES_AUTH.

import { randomUUID } from 'node:crypto';
import { request } from 'undici';

import { getAccessToken, requiresAuth, API_BASE_URL } from './jpmAuth.js';
...
// The mock endpoint is unauthenticated. Keyed off the Checkout URL rather
// than API_BASE_URL, since JPM_CHECKOUT_API_URL can point somewhere else.
const REQUIRES_AUTH = requiresAuth(CHECKOUT_API_URL);
 ...
 *
 * The Authorization header is omitted against the mock environment, which
 * takes requests unauthenticated.
 */
...
export { CHECKOUT_API_URL, REQUIRES_AUTH };
