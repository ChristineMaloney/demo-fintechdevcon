...
// so it is served from whichever environment this integration targets —
// the mock serves its own alongside the REST and frontend-service
// surfaces. Set JPM_DROPIN_UI_URL to override.
const BUNDLE_PATH = '/drop-in-ui.mjs';
const CAT_BUNDLE_ORIGIN = 'https://checkout-cat.merchant.jpmorgan.com';

export function dropInUiUrl() {
  if (process.env.JPM_DROPIN_UI_URL) return process.env.JPM_DROPIN_UI_URL;
  const origin = isMockTarget() ? new URL(CHECKOUT_API_URL).origin : CAT_BUNDLE_ORIGIN;
  return `${origin}${BUNDLE_PATH}`;
}
