...
// The browser-side Drop-in UI bundle. JPM publishes it for CAT and PROD
// only — the mock serves the REST and frontend-service surfaces but not
// the bundle, so a mock target loads the CAT build and has its backend
// calls redirected to the mock instead (see frontendServiceBase).
// Set JPM_DROPIN_UI_URL to override.
const CAT_BUNDLE = 'https://checkout-cat.merchant.jpmorgan.com/drop-in-ui.mjs';
const PROD_BUNDLE = 'https://checkout.merchant.jpmorgan.com/drop-in-ui.mjs';
const PROD_CHECKOUT_HOST = 'merchant-api.checkout.merchant.jpmorgan.com';

export function dropInUiUrl() {
  if (process.env.JPM_DROPIN_UI_URL) return process.env.JPM_DROPIN_UI_URL;
  return new URL(CHECKOUT_API_URL).hostname === PROD_CHECKOUT_HOST
    ? PROD_BUNDLE
    : CAT_BUNDLE;
}
...
export function resolveEndpoints() {
  const bundle = dropInUiUrl();
  const offTarget =
    isMockTarget() && new URL(bundle).hostname !== new URL(CHECKOUT_API_URL).hostname;
  ...
  return {
    ...
    frontendService: frontendServiceBase() ?? '(bundle default)',
    dropInBundle: offTarget ? `${bundle}  (not served by the mock; calls redirected)` : bundle,
    token: isMockTarget() ? '(not used — mock takes no credentials)' : TOKEN_URL,
  };
}

/**
 * Base the browser-side Drop-in bundle should send its `/checkout.fe.v1.*`
 * frontend-service calls to, or null to leave the bundle's own baked-in
 * backend alone.
 *
 ...
 *
 * This is the full base including any path prefix, not just the origin:
 * the mock serves these under the same /v1 as the REST endpoints, and
 * redirecting to the bare origin gets a 404. Override with
 * JPM_FRONTEND_SERVICE_URL if an environment nests them elsewhere.
 */
export function frontendServiceBase() {
  if (process.env.JPM_FRONTEND_SERVICE_URL) {
    return trimSlash(process.env.JPM_FRONTEND_SERVICE_URL);
  }
