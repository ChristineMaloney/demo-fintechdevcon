...
/**
 * Skips OAuth entirely — no JWT is signed and no token is fetched. Only for
 * JPM's mock endpoint, which accepts unauthenticated requests and returns
 * canned responses. CAT and PROD both reject unauthenticated calls, so this
 * must be false anywhere real money or real test credentials are involved.
 */
function bypassAuth() {
  return process.env.JPM_BYPASS_AUTH === 'true';
}
...
export function isConfigured() {
  if (!process.env.JPM_PAYMENTS_API_URL || !process.env.JPM_MERCHANT_ID) {
    return false;
  }
  // Against the mock there are no credentials to check.
...
async function request(method, path, payload) {
  ...
  // No token is signed or fetched in bypass mode — getAccessToken() is never
  // reached, so the absence of credentials is not an error there.
  if (!bypassAuth()) {
